...
/* Remarque : à adapter au code du haut*/
void solve_jacobi(mesh_t* A, mesh_t const* B, mesh_t* C) {
    assert(A->dim_x == B->dim_x && B->dim_x == C->dim_x);
    assert(A->dim_y == B->dim_y && B->dim_y == C->dim_y);
    assert(A->dim_z == B->dim_z && B->dim_z == C->dim_z);
    ...
    for (usz i = STENCIL_ORDER; i < dim_x - STENCIL_ORDER; ++i)  {
        for (usz j = STENCIL_ORDER; j < dim_y - STENCIL_ORDER; ++j) {
            for (usz k = STENCIL_ORDER; k < dim_z - STENCIL_ORDER; ++k){
                C->cells_value[i*dim_y*dim_z + j*dim_z + k] = A->cells_value[i*dim_y*dim_z + j*dim_z + k] * B->cells_value[i*dim_y*dim_z + j*dim_z + k];

                for (usz o = 1; o <= STENCIL_ORDER; ++o) {
                    tmp = 1 / pow(17.0, (f64)o);

                    C->cells_value[i*dim_y*dim_z + j*dim_z + k] += A->cells_value[(i + o)*dim_y*dim_z + j*dim_z + k] *
                                                             B->cells_value[(i + o)*dim_y*dim_z + j*dim_z + k] * tmp;

                    C->cells_value[i*dim_y*dim_z + j*dim_z + k] += A->cells_value[(i - o)*dim_y*dim_z + j*dim_z + k] *
                                                             B->cells_value[(i - o)*dim_y*dim_z + j*dim_z + k] * tmp;

                    C->cells_value[i*dim_y*dim_z + j*dim_z + k] += A->cells_value[i*dim_y*dim_z + (j + o)*dim_z + k] *
                                                             B->cells_value[i*dim_y*dim_z + (j + o)*dim_z + k] * tmp;

                    C->cells_value[i*dim_y*dim_z + j*dim_z + k] += A->cells_value[i*dim_y*dim_z + (j - o)*dim_z + k] *
                                                             B->cells_value[i*dim_y*dim_z + (j - o)*dim_z + k] * tmp;

                    C->cells_value[i*dim_y*dim_z + j*dim_z + k] += A->cells_value[i*dim_y*dim_z + j*dim_z + (k+o)] *
                                                             B->cells_value[i*dim_y*dim_z + j*dim_z + (k+o)] * tmp;
