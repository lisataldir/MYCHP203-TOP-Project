    ...
    for (usz i = STENCIL_ORDER; i < dim_x - STENCIL_ORDER; ++i)  {
        for (usz j = STENCIL_ORDER; j < dim_y - STENCIL_ORDER; ++j) {
            for (usz k = STENCIL_ORDER; k < dim_z - STENCIL_ORDER; ++k){
                tmp = A->cells[i][j][k].value * B->cells[i][j][k].value;

                for (usz o = 1; o <= STENCIL_ORDER; ++o) {
                    tmp += A->cells[i + o][j][k].value *
                            B->cells[i + o][j][k].value * o_values[o - 1];
                    tmp += A->cells[i - o][j][k].value *
                            B->cells[i - o][j][k].value * o_values[o - 1];
                    tmp += A->cells[i][j + o][k].value *
                            B->cells[i][j + o][k].value * o_values[o - 1];
                    tmp += A->cells[i][j - o][k].value *
                            B->cells[i][j - o][k].value * o_values[o - 1];
                    tmp += A->cells[i][j][k + o].value *
                            B->cells[i][j][k + o].value * o_values[o - 1];
                    tmp += A->cells[i][j][k - o].value *
                            B->cells[i][j][k - o].value * o_values[o - 1];
                }

                C->cells[i][j][k].value = tmp;
            }
        }
    }
